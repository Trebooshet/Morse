...
let index = 0;
let dotSound = new Audio('/Users/trebooshet/Desktop/HTML/Морзе/1soundpip.mp3')
let dashSound = new Audio('/Users/trebooshet/Desktop/HTML/Морзе/3soundpip.mp3')
const symbols = [
...
async function readMorse(morseText) {
    ...
    if (char === '.') {
        ...
        dotSound.play();
        delay = 400; // Пауза для точки
    } else if (char === '-') {
        body.style.backgroundImage = "url('2.jpg')";
        dashSound.play();
        delay = 1200; // Пауза для тире
    } else if (char === ' ') {
        body.style.backgroundImage = "url('1.jpg')";
        delay = 2800; // Пауза для пробела
    }
    ...
    await sleep(400); // Ожидаем перед тем как вызвать следующий символ
    readMorse(morseText);
